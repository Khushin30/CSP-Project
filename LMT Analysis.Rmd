...
```
#### It seems like all of them have a very small correlation with net_change. Maybe we will see a correlation if we combine them all into just one variable that signifies that there was some solar event occuring on that day. Lets first consider how it is distributed if we do all of the initial rows
...
```
#### shoes no correlation at all lets try the same thing with the balanced dataset
```{r}
...
```
##### solar seems to be the only one that has a small correlation
...
rpart.plot(tree_balanced, main = "Balanced Decision Tree for LMT")

tree_processed <- rpart(cat_change ~ ., data = train_processed_data, method = "class")
rpart.plot(tree_processed, main = "Regular Decision Tree for LMT")
```
...
rf_model <- randomForest(cat_change ~ ., data = train_processed_data, method = "class")
importance <- importance(rf_model)
importance
# Plot feature importance
varImpPlot(rf_model, main = "Unbalanced Random Forest LMT")
```
